/**
 * In-memory + optional Redis store. Defaults to memory when REDIS_HOST absent.
 */
...
function init() {
  ...
  if (!host) { useMemory = true; logger.warn('⚠️  REDIS_HOST not set; using in-memory'); return null; }
  client = new Redis({ host, port, retryStrategy: (t) => Math.min(t * 200, 2000), maxRetriesPerRequest: 3, lazyConnect: false });
  client.on('connect', () => logger.info('🔌 Redis connected'));
  client.on('error', (e) => { logger.error('❌ Redis error:', e.message); useMemory = true; });
  return client;
}
...
async function ltrim(k, s, e) { if (useMemory) { const arr = memory.get(k) || []; memory.set(k, arr.slice(s, e === -1 ? undefined : e + 1)); return 'OK'; } return client.ltrim(k, s, e); }
async function keys(p) { if (useMemory) { const re = new RegExp(p.replace(/\*/g, '.*')); return Array.from(memory.keys()).filter((k) => re.test(k)); } return client.keys(p); }
async function del(k) { return useMemory ? (memory.delete(k) ? 1 : 0) : client.del(k); }

module.exports = { init, get, set, hset, hgetall, hdel, lpush, lrange, ltrim, keys, del, client: () => client };
